...
export default function PatientPlanosTab({ patient }: { patient: Patient }) {
  const { api } = useAuth();
  ...
  const plansQuery = useQuery<TreatmentPlan[]>(`/treatment-plans?patientId=${pid}`);
  ...
  async function create() {
    if (!form.title || !patient) return;
    setSaving(true);
    setErroEscrita('');
    try {
      await api('/treatment-plans', {
        method: 'POST',
        body: { patientId: patient.id, ...form, totalFee },
      });
      plansQuery.refetch();
      setModal(false);
          ...
          {form.phases.map((ph, i) => (
            ...
            <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', marginBottom: 8 }}>
              ...
              {form.phases.length > 1 && (
                <GhostBtn style={{ padding: '4px 8px', fontSize: 'var(--text-2xs)' }} onClick={() => removePhase(i)}>
                  ×
                </GhostBtn>
              )}
            </div>
