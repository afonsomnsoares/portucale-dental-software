...
import type { LeadCaptureSource, LeadCaptureSourceWithToken, Tenant } from '@/lib/types';
...
export default function LeadSourcesPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    const qs = isSuperAdmin ? `?tenantId=${tenantId}` : '';
    const rows = await api(`/lead-sources${qs}`).catch(() => []);
    setSources(rows || []);
    setLoading(false);
  ...
  async function create() {
    ...
    try {
      const row = await api('/lead-sources', {
        method: 'POST',
        body: { label: label.trim(), ...(isSuperAdmin ? { tenantId } : {}) },
      });
