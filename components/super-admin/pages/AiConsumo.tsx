...
// ─── «Modelos» e «Custos de IA» eram a mesma leitura ────────────────────────
// Duas entradas de menu, dois componentes, e o mesmo /platform/ai-usage por baixo — com
// a mesma tabela por agente desenhada nos dois sítios, uma delas com menos colunas. Não
// eram duas perguntas: era uma resposta partida ao meio, e a metade de cima («que modelo
// usamos») cabe numa ficha por cima da outra.
//
// O que se perdeu na fusão foi a tabela pobre: a versão de «Modelos» mostrava agente,
// modelo, chamadas, falhas e duração, que são cinco das oito colunas que a de «Custos»
// já tinha. Ficou a completa.
//
// O modelo não é configurável por clínica nem por agente: é uma constante em
// lib/agents/aiClient.ts (AGENT_MODEL), escolhida com uma razão escrita ao lado dela. A
// tabela de modelos com utilização registada mostra também os antigos, que continuam em
// ai_calls depois de a constante mudar — é assim que se vê que uma troca de modelo
// aconteceu mesmo, e quando.
const n = (v: string | number) => Number(v || 0);
const fmtTok = (v: string | number) => n(v).toLocaleString('pt-PT');

export default function AiConsumo({ initialData }: { initialData?: AiUsage } = {}) {
    ...
    <div className="card p-5" style={{ marginBottom: 16 }}>
      ...
      {price && (
        <div style={{ fontSize: 'var(--text-sm)', color: 'var(--text-secondary)' }}>
          {price.input} € por milhão de tokens de entrada · {price.output} € por milhão de saída
        </div>
      )}
      <p
        style={{
      ...
      >
        ...
        correm em segundo plano, uma por clínica por passagem do cron, e são decisões estruturadas e limitadas — o custo
        por corrida pesa mais aqui do que a capacidade bruta.
      </p>
    </div>
  );
