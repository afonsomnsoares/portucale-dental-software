...
const EXPIRY_META: Record<string, { label: string; bg: string; color: string }> = {
  expired: { label: 'Expirado', bg: 'var(--red-bg)', color: 'var(--red)' },
  expiring_soon: { label: 'A expirar', bg: 'var(--amber-bg)', color: 'var(--amber)' },
  ok: { label: 'Válido', bg: 'var(--green-bg)', color: 'var(--green)' },
  no_expiry: { label: 'Sem validade', bg: 'var(--surface-2)', color: 'var(--ink-2)' },
};
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14, marginBottom: 20 }}>
        <MetricCard label="ITENS EM RISCO DE RUTURA" value={atRiskCount} color="#DE350B" />
        <MetricCard label="LOTES A EXPIRAR / EXPIRADOS" value={expiringCount} color="#FF8B00" />
        <MetricCard label="ITENS RASTREADOS" value={rows.length} color="#0052CC" />
      </div>
        ...
        <div className="card mb-5" style={{ padding: 0 }}>
          <DataTable
            ...
            rows={rows.map((r) => (
              <tr key={r.item.id}>
                <TD bold>
                  {r.item.item} <span style={{ color: 'var(--ink-3)', fontWeight: 400 }}>({r.item.unit})</span>
                </TD>
                <TD>{r.currentQty}</TD>
                ...
                <TD>
                  {r.atRisk ? (
                    <Badge label={`Repor ${r.suggestedReorderQty}`} bg="var(--red-bg)" color="var(--red)" />
                  ) : (
                    <Badge label="OK" bg="var(--green-bg)" color="var(--green)" />
                  )}
                </TD>
                <TD>
                  {!r.batches.length ? (
        ...
        {!data.procedureDemand.length ? (
          ...
          <DataTable
            ...
            rows={data.procedureDemand.map((d) => (
              <tr key={d.itemId}>
                <TD bold>
                  {d.item} <span style={{ color: 'var(--ink-3)', fontWeight: 400 }}>({d.unit})</span>
                </TD>
                <TD>{d.currentQty}</TD>
                <TD>{d.projectedDemand}</TD>
                <TD>
                  {d.shortfall > 0 ? (
                    <Badge label={`Faltam ${d.shortfall}`} bg="var(--red-bg)" color="var(--red)" />
                  ) : (
                    <Badge label="Coberto" bg="var(--green-bg)" color="var(--green)" />
                  )}
                </TD>
              </tr>
            ))}
      ...
      <div className="card p-4 mb-3">
        ...
        {error && <div style={{ fontSize: 12, color: '#DE350B', fontWeight: 700, marginTop: 8 }}>{error}</div>}
      </div>
      {usage.length > 0 && (
