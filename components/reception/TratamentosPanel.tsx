...
export default function TratamentosPanel() {
  ...
  return (
    <div>
      ...
      {/* O cifrão estava escrito à mão — `$${'{'}totalFee{'}'}` — num produto que fatura em
          euros. Passou a formatEUR (lib/constants.ts), que é quem sabe pôr o
          símbolo do lado certo e com o separador de milhares português. */}
      <MetricStrip>
        <Metric label="valor_total" value={formatEUR(totalFee)} sub="em planos abertos" />
        <Metric
          label="propostos"
          value={proposed}
          sub="à espera de resposta do doente"
          urgency={proposed > 0 ? 'soon' : undefined}
        />
        <Metric label="aceites" value={accepted} sub="com consulta marcada" />
        <Metric label="concluidos" value={completed} sub="tratamentos terminados" />
      </MetricStrip>

      {/* ─── Os filtros são ferramentas da lista, não uma barra solta ──────────
          Estavam numa fila própria por cima do cartão, e o botão de criar noutra
          fila por cima dessa. Três filas de controlos antes de se ver um único
          tratamento. Agora vivem na cabeça da secção que filtram — que é onde
          também está a contagem do que eles deixaram passar. */}
      <WorkSection
        title="Planos de tratamento"
        count={`${visible.length} de ${treatments.length}`}
        tools={<PrimaryBtn onClick={() => setModal(true)}>+ Novo tratamento</PrimaryBtn>}
      >
