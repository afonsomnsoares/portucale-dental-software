...
export default function FaturasPanel() {
  const { api } = useAuth();
  ...
  // ─── Cifrões num produto que fatura em euros ──────────────────────────────
  // Estavam escritos à mão — `$${'{'}valor.toLocaleString(){'}'}` — em cinco sítios
  // deste ecrã, e `toLocaleString()` sem argumentos segue a locale do BROWSER:
  // num portátil configurado em inglês, «1.234,50» saía «1,234.5». O formatEUR de
  // lib/constants.ts fixa pt-PT e põe o símbolo certo do lado certo.
  function invTotal(inv: Invoice) {
    return formatEUR(Number(inv.amount));
  }
  function invPaid(inv: Invoice) {
    return formatEUR(Number(inv.paid));
  }
  function invBal(inv: Invoice) {
    return formatEUR(Math.max(0, Number(inv.amount) - Number(inv.paid)));
  }
  ...
  return (
    <div>
      <PanelBar>
        ...
        <GhostBtn onClick={() => setModal(true)}>+ Nova fatura</GhostBtn>
      </PanelBar>

      <PanelNote>
        {totals.count} fatura{totals.count !== 1 ? 's' : ''} · {formatEUR(totals.amount)} faturado ·{' '}
        {formatEUR(totals.paid)} recolhido
      </PanelNote>
