'use client';
// ─── Recalls ────────────────────────────────────────────────────────────────
// Partilhado entre a receção (que os agenda e fecha) e a direção (que os vê como receita
// por recuperar). Era uma página inline de 283 linhas na árvore da receção, e a direção
// não lhe chegava de todo.
//
...
export default function Recalls() {
  ...
  function statusMeta(r: Recall) {
    if (!r.active) return { label: 'Inativo', bg: 'var(--urgency-critical-bg)', color: 'var(--urgency-critical)' };
    if (r.next_due && r.next_due < new Date().toISOString().slice(0, 10))
      return { label: 'Em atraso', bg: 'var(--urgency-soon-bg)', color: 'var(--urgency-soon)' };
    if (r.last_done) return { label: 'Ativo', bg: 'var(--urgency-ok-bg)', color: 'var(--urgency-ok)' };
    return { label: 'Ativo', bg: 'var(--urgency-ok-bg)', color: 'var(--urgency-ok)' };
  }
