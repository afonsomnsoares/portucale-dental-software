...
export async function GET(request: NextRequest) {
  ...
  return Response.json({
    ...
    byTenant: byTenant.map((r) => ({ ...r, costEur: cost(Number(r.input_tokens), Number(r.output_tokens)) })),
    byAgent: byAgent.map((r) => ({ ...r, costEur: cost(Number(r.input_tokens), Number(r.output_tokens), r.model) })),
